...
export const AppHOF = () => {
  const fiter = (_: Auction): Boolean => {
    return true;
  };
  const auctions = useObservedAuction({ filter: fiter });
  const [view, setView] = useState<ViewState>({
    type: "DEFAULT",
    component: ({ auction, onClick }) => (
      <ListAuction key={auction.id} auction={auction} onClick={onClick} />
    ),
  });
  ...
  return (
    <React.StrictMode>
      <HideOnScroll>
        <AppBar>
          <Toolbar sx={{ background: "#011F26" }}>{nav(auctions)}</Toolbar>
        </AppBar>
      </HideOnScroll>
